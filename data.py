...
def remove_cross_split_duplicates(
    ...
    keep = [
        index
        for index, document in enumerate(eval_docs)
    ...
    for category_entry in categories:
        ...
        for file_entry in files:
            ...
            if progress and processed % 1000 == 0:
                print(
                    f"Newsgroups files processed: {processed}; documents loaded: {len(docs)}",
                    flush=True,
                )
...
def load_reuters(path, split=None, return_topics=False, progress=True):
    ...
    with os.scandir(path) as entries:
        sgm_files = sorted(
            [entry.name for entry in entries if entry.is_file() and entry.name.endswith(".sgm")],
        )
    ...
    for file_index, file in enumerate(sgm_files, start=1):
        with open(os.path.join(path, file), encoding="latin-1") as f:
            ...
            for article in soup.find_all("reuters"):
                ...
                topic_names = (
                    tuple(
                        sorted(
                            {tag.get_text(strip=True).casefold() for tag in topics.find_all("d")}
                        )
                    )
                    if topics
                    else ()
        ...
        if progress:
            print(
                f"Reuters files processed: {file_index}/{len(sgm_files)}; documents loaded: {len(docs)}",
                flush=True,
            )
...
def split_documents(docs, labels, eval_fraction=0.2, seed=42):
    ...
    for lab in np.unique(labels):
        ...
        n_eval = 0 if eval_fraction == 0 else max(1, int(round(len(class_indices) * eval_fraction)))
