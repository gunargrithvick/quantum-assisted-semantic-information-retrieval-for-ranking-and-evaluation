...
def train_qsvm(embeddings, labels, topic_sets=None):
    ...
    if qsvm_multilabel:
        ...
        qsvm_model = OneVsRestClassifier(SVC(kernel="precomputed", decision_function_shape="ovr"))
    else:
...
def _pair_is_relevant(query_index, document_index, labels, topic_sets=None):
    ...
    return bool(set(topic_sets[query_index]).intersection(topic_sets[document_index]))
...
def train_quantum_relevance_ranker(
    ...
    for query_index in query_indices:
        ...
        positive_indices = rng.choice(positives, size=min(per_side, len(positives)), replace=False)
        negative_indices = rng.choice(negatives, size=min(per_side, len(negatives)), replace=False)
...
class _RestrictedUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        if (module, name) not in self._allowed_globals:
            raise pickle.UnpicklingError(f"Blocked model-state class: {module}.{name}")
...
def _transactional_pipeline(function):
    ...
    def wrapped(*args, **kwargs):
        previous_state = {name: globals()[name] for name in _RUNTIME_STATE_NAMES}
...
def save_model_state(dataset_name, docs, labels):
    ...
    required = (
        ...
        and cluster_pca is not None
        and cluster_scaler is not None
        and kmeans_model is not None
        and cluster_labels is not None
        and qsvm_model is not None
        and qsvm_pca is not None
        and qsvm_scaler is not None
        and qsvm_train_reduced is not None
        and ranker_model is not None
        and ranker_scaler is not None
        and ranker_train_features is not None
    )
...
def _build_tfidf_index():
    ...
    tfidf_matrix = tfidf_vectorizer.fit_transform(clean_text(document) for document in docs)
...
def retrieve_candidates(query, k=CANDIDATE_POOL_SIZE):
    ...
    cluster_ready = (
        ...
        and cluster_pca is not None
        and cluster_scaler is not None
        and len(cluster_labels) == index.ntotal
    )
...
def _classical_candidate_scores(query, candidates=None):
    ...
    classical_scores = normalize_scores(cosine_similarity(q_embed, candidate_embeddings)[0])
...
def lexical_baseline_ranking(query, k=25, candidates=None):
    ...
    overlap_scores = np.fromiter(
        (len(query_terms.intersection(set(clean_text(document).split()))) for document in docs),
        dtype=float,
        count=len(docs),
    )
...
def _quantum_component_scores(query, candidates=None):
    ...
    quantum_state_ready = (
        qsvm_pca is not None
        and qsvm_scaler is not None
        and qsvm_kernel is not None
        and qsvm_train_reduced is not None
        and ranker_scaler is not None
        and ranker_kernel is not None
        and ranker_model is not None
        and ranker_train_features is not None
    )
    ...
    pair_kernel = ranker_kernel.evaluate(pair_features.tolist(), ranker_train_features.tolist())
    ...
    if qsvm_model is None:
        ...
        decision_scores = np.asarray(qsvm_model.decision_function(candidate_train_kernel))

        if qsvm_multilabel:
            query_topic_scores = np.asarray(qsvm_model.decision_function(query_train_kernel))
            ...
            if decision_scores.ndim == 1:
                ...
                qsvm_scores = (
                    decision_scores if predicted_class == positive_class else -decision_scores
                )
            else:
    ...
    quantum_score_cache[cache_key] = (
        candidates.copy(),
        tuple(component.copy() for component in components),
    )
...
def tune_ranking_weights(
    query_docs, query_labels, corpus_labels, query_topics=None, corpus_topics=None, n_per_class=1
):
...
def _evaluate_queries(eval_queries, verbose=False, reveal_queries=False):
    ...
    for q in eval_queries:
        ...
        candidate_recalls.append(recall_at_k(candidate_pool, relevant, CANDIDATE_POOL_SIZE))
        ...
        if verbose:
            query_display = f' "{format_snippet(q["query"], 60)}"' if reveal_queries else ""
...
def evaluate_model(n_per_class=2, verbose=True, reveal_queries=False, query_seed=42):
    ...
    eval_queries = build_eval_queries(
        ...
        eval_labels,
        labels,
        query_topics=eval_topics,
        corpus_topics=doc_topics,
        n_per_class=n_per_class,
        seed=query_seed,
    )
    ...
    for method, values in results_by_method.items():
        ...
        print(
            "{:<24} {:>12.4f} {:>12.4f} {:>12.4f} {:>12.4f}".format(
...
def evaluate_across_seeds(seeds=(42, 43, 44), n_per_class=2):
    ...
    multi_seed_results[current_dataset] = {
        "seeds": list(seeds),
        "queries_per_seed": [count for _, _, count in per_seed],
        "methods": summary,
    }
...
def show_metrics():
    ...
    if comparison_results:
        ...
        source = {
            dataset: {"Quantum-assisted": values} for dataset, values in dataset_results.items()
        }
...
def train_newsgroups_model():
    ...
    raw_embeddings, model = generate_embeddings(docs, f"{current_dataset}_train", labels=labels)
...
def train_reuters_model():
    ...
    (docs, labels, eval_docs, eval_labels, doc_topics, eval_topics) = remove_cross_split_duplicates(
        docs,
        labels,
        eval_docs,
        eval_labels,
        train_topics=doc_topics,
    ...
    ranking_weights, objective, validation_query_count = tune_ranking_weights(
        ...
        validation_labels,
        fit_labels,
        query_topics=validation_topics,
        corpus_topics=fit_topics,
        n_per_class=1,
    )
    print(
        "Validation tuning complete:",
    ...
    print("Refitting final pipeline on all training documents...", flush=True)
    _fit_pipeline(full_train_docs, full_train_labels, raw_embeddings, 200, full_train_topics)
    docs = full_train_docs
    labels = full_train_labels
...
def rebuild_model_for_search(choice):
    ...
    embeddings, model = generate_embeddings(docs, f"{current_dataset}_train", labels=labels)
