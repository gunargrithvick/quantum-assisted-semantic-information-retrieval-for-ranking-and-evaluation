...
For the full local quality checks used by continuous integration, install the
development requirements and run:
...
This release is a local research and demonstration CLI. It has no web server,
API route, hosted demo, or Vercel configuration. Deploying it as a web
application would require a separate UI/API layer plus a plan for the large
embedding and quantum-simulation dependencies, dataset storage, model caching,
...
- QSVM training uses a deterministic subset of the training set to limit kernel
  computation cost.
- The quantum-assisted ranker is not presented as superior to the classical
  baselines; the recorded runs show it below the classical semantic baseline.
